...
def stuClassRanking(stuName):
    if stuNameIsIn(stuName):
        dfResult = dataToPD.getTheStuResults(stuName)
        dfResultList = dfResult.values[0].tolist()  # 将 查询学生的成绩 转化成列表
        listX = []
        for i in range(5):
            listX.append(dataToPD.getStuAllRank(stuName)[0][i]+'\n'+str(dfResultList[i]))
        bar = Bar()
        bar.add_xaxis(listX)
...
def radar4Line(stuName):                                    #绘制雷达图
    dataTable = dataToPD.getRadarTable(stuName)
...
def toVarianceBar(stuName, courseName,vip=False):                     #学科均分的柱状图
    dfBase = dataToPD.getBaseDf()
    x = '班级'
    if vip:
        groupXX = dataToPD.getTheCourseVariance(courseName, choose=True)
    else:
        groupXX = dataToPD.getTheCourseVariance(courseName)
...
def lineTotalScore(courseName):                                      #某学科 多少名之前的总分的趋势
    dfSum = dataToPD.scoreClassSum(courseName=courseName)
    attr = dfSum[1]                                                     #x轴的列表
    c = Line().add_xaxis(attr).set_global_opts(
        title_opts=opts.TitleOpts(title="", subtitle=""),
        tooltip_opts=opts.TooltipOpts(trigger="axis"),toolbox_opts=opts.ToolboxOpts(is_show=False),
